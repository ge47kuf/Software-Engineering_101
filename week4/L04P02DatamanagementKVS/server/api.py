...
def get(key):
    ...
    If there is no user-id `uid` given in the request body or the user-id does not match,
...
def put(key):
    ...
    If there is no user-id `uid` or file-content `content` given in the request body,
    return error message and status.
    ...
    ff.put(key, Entry(uid=uid, content=content))
...
def list():
    ...
    for key, entry in ff.items():
        if entry.uid == uid:
            result.append(f"{key}:{entry.content}")

    return Response(", ".join(result), status=OK)
