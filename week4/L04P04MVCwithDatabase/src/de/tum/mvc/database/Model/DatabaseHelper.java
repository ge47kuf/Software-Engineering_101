...
public class DatabaseHelper {
    ...
    public synchronized Product updateProduct(Product product) {
        //1.create PreSta: UPDATE <tablename> SET <stuffTo Set> WHERE condition
        String updateSQL = "UPDATE " + PRODUCT_TABLE + " SET name = ?, description = ?, "
                + "price = ?, stored = ? WHERE id = ?";

        //2.connect to DB
        try (
                PreparedStatement p = conn.prepareStatement(updateSQL, Statement.RETURN_GENERATED_KEYS)) {
        //3. set parameter
            p.setString(1, product.getName());
            p.setString(2, product.getDescription());
            p.setDouble(3, product.getPrice());
            p.setInt(4, product.getStoredQuantity());
            p.setInt(5, product.getId());
        //4. update
            int isUpdated = p.executeUpdate();
        //5. check if update fail
            if (isUpdated == 1) {
