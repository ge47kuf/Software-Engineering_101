...
public class PersonResource {
    ...
    public ResponseEntity<Person> deletePerson(@PathVariable("personId") UUID deletedId) {
        if (deletedId == null) {
            return ResponseEntity.badRequest().build();
        }
        personService.deletePerson(deletedId);
        return ResponseEntity.noContent().build();
    }

    @GetMapping("persons")
    public ResponseEntity<List<Person>> getAllPersons(@RequestParam(value = "sortField", defaultValue = "ID")
                                                      PersonSortingOptions.SortField sortField,
                                                      @RequestParam(value = "sortingOrder", defaultValue = "ASCENDING")
                                                      PersonSortingOptions.SortingOrder sortingOrder) {
