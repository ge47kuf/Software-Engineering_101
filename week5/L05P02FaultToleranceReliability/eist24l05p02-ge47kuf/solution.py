...
def log_and_apply_operations(operation_list, store, log_file):
    ...
    Args:
        operation_list (list): List of operations to log and apply.
...
def apply_operation(operation, store):
    ...
    ## TODO
    if key not in store: store[key] = '0'

    # operation handline here
    if action == "set":
...
def main(initial_kv_store, operation_list_list, undo_operation_list_list, redo_log_file, undo_log_file):
    ...
    Args:
        ...
        operation_list_list (list): List of list of operations to perform.
        undo_operation_list_list (list): List to store undo operations.
    ...
    for operation_list in reversed(operation_list_list):  # Reverse the order for undo operations
        ...
        for operation in reversed(operation_list):
            ...
            undo_op = {"action": "", "key": key, "value": ""}
            if action == "set":
                if key in initial_kv_store:
                    undo_op["action"] = "set"
                    undo_op["value"] = initial_kv_store[key]
                else:
                    undo_op["action"] = "delete"
            elif action == "delete":
                if key in kv_store:
                    undo_op["action"] = "set"
                    undo_op["value"] = kv_store[key]
            elif action == "add":
                undo_op["action"] = "subtract"
                undo_op["value"] = operation["value"]
            elif action == "subtract":
                undo_op["action"] = "add"
                undo_op["value"] = operation["value"]
            elif action == "multiply":
                undo_op["action"] = "divide"
                undo_op["value"] = operation["value"]
            elif action == "divide":
                undo_op["action"] = "multiply"
                undo_op["value"] = operation["value"]
