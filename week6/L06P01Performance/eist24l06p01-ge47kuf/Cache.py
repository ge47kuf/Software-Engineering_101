...
class Cache:
    ...
    def _evict(self):
        ...
        # case distinction equal if else
        if self.policy == 'fifo':
            self.cacheStore.popitem(last=False)  # true = lifo, false = fifo
        elif self.policy == 'lru':  # rm least recent use
            lr_key = min(self.counter, key=self.counter.get)  # key= function die auf self.counter angewendet werden
            del self.cacheStore[lr_key]  # rm 'a' aus self.cacheStore
            del self.counter[lr_key]     # rm 'a' aus self.counter
        elif self.policy == 'lfu':  #  least frequently use
            lfu_key = min(self.counter, key=self.counter.get)
            del self.cacheStore[lfu_key]
            del self.counter[lfu_key]
        elif self.policy == 'mru':
            mru_key = max(self.counter, key=self.counter.get)
            del self.cacheStore[mru_key]
            del self.counter[mru_key]
        elif self.policy == 'mfu':
            mfu_key = max(self.counter, key=self.counter.get)
            del self.cacheStore[mfu_key]
...
##############################################################
    ...
    def modify_counter(self, key):
        ...
        if self.policy in ['lru', 'mru', 'lfu', 'mfu']:
            self.counter[key] += 1
        ##############################################################
    ...
    def get(self, key):
        ...
        ##############################################################
        # YOUR CODE HERE:
        # TODO: Implement the case if key is in the cache, modify the counter according to
        #  different policies
    ...
    def put(self, key, value):
        ...
        # Hint: You have already implemented the modify_counter method.
        if key not in self.cacheStore and len(self.cacheStore) >= self.size:
            self._evict()
